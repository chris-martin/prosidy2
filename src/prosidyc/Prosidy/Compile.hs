...
import Prosidy.Compile.Internal.Eval (Eval(Eval), EvalError(..), exhaustive)
...
block name description onTag onLit onPg = do
    ...
    annotateProduct <$> chooseOne key description [onTagRule, onLitRule, onPgRule]
  where
...
inline name description onTag onText onBreak = do
    ...
    annotateProduct <$> chooseOne key description [onTagRule, onTextRule, onBreakRule]
  where
...
paragraph name desc =
    fmap annotateProduct . productRule key desc
  where
    key = infoKey @P.Paragraph name
...
break name description replace =
    productRule key description (embed replace)
  where
    key = infoKey @() name
...
literal name description transform = 
    fmap annotateProduct
    <$> productRule key description $ access (transform . view P.content)
  where
    key = infoKey @P.Literal name
...
fragment name description transform = 
    ...
    <$> productRule key description $ access (transform . view P.content)
  where
    key = infoKey @Text name

oneOf :: forall input output context.
...
-------------------------------------------------------------------------------
child :: (Monad context, P.FromSource input, P.HasContent input) => 
    ProductRule (P.Content input) context output ->
    Desc input context output
child = descendWith P.content

children :: (Traversable t, Monoid output, Monad context, P.FromSource input, P.HasContent input, P.Content input ~ t x) =>
